                ...
                <div class="row">
                    ...
                    <div class="col-md-4 col-xl-8" style="padding-left: 43px;">
                        <div style="text-align:center;">
                            <h2 class="divider-style" style="margin-top: -1px;"><span>Booking Attendance Rate<br></span></h2>
                        </div>
                        <div class="form-group pull-right">
                            <input type="text" class="search form-control" placeholder="What you looking for?">
                        </div>
                        <span class="counter pull-right"></span>
                        <label>User id:</label> <label>${param.memberId}</label><br/>
                        <label>Total Approve Booking: </label> <label><bookingRecords:showBookingRecord tagType="showTotalApproveBookingNumber" memberId="${param.memberId}" radio="${param.radio}" year="${param.year}" month="${param.month}" /></label><br/>
                        <label>Present Booking: </label> <label><bookingRecords:showBookingRecord tagType="showPresendBookingNumber" memberId="${param.memberId}" radio="${param.radio}" year="${param.year}" month="${param.month}" /></label><br/>
                        <label>Absent Booking: </label> <label><bookingRecords:showBookingRecord tagType="showAbsentBookingNumber" memberId="${param.memberId}" radio="${param.radio}" year="${param.year}" month="${param.month}" /></label><br/>
                        <label>Booking attendance rate: </label> <label><bookingRecords:showBookingRecord tagType="showBookingAttendanceRate" memberId="${param.memberId}" radio="${param.radio}" year="${param.year}" month="${param.month}" /></label>
                        <table class="table table-hover table-bordered results">
                            ...
                            <tbody>
                                <bookingRecords:showBookingRecord tagType="showTotalApproveBookingByTable" memberId="${param.memberId}" radio="${param.radio}" year="${param.year}" month="${param.month}" />
                            </tbody>
                        </table></div>
                </div>
            </div>
